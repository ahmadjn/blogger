...
function is_static_url($link)
{  
  $filter_path = arsae_config()->whitelists;

  $sketch = '/.*' . implode('.*|.*', $filter_path) . '.*/i';

  return preg_match($sketch, $link);  
}
...
function proxify($url)
{
    ...
    foreach (
        ...
        if (!is_static($oldLink) && !is_static_url($oldLink)) {
                    $link->setAttribute(
                        "href",
                        arsae_config()->server . "?arsae=" . urlencode($oldLink)
                    );
                }
        else if(!is_static_url($oldLink))
        {
            $link->setAttribute(
                "href",
                arsae_config()->server .
